// Default Tailwind CSS color palette (v3) + a nearest-color matcher.
//
// DevTools reports colors as computed rgb(), which the mapper converts to hex.
// A raw hex produces an arbitrary class like `text-[#3b82f6]`. This module lets
// the mapper recognise when that hex IS a named Tailwind color and emit the
// readable `text-blue-500` instead. It only snaps when the match is essentially
// exact (see SNAP_MAX_DELTA) — a deliberately custom color is left as-is rather
// than silently shifted to the nearest named token.

/** name (e.g. "blue-500", "white") → lowercase hex. */
...
/**
 * Max per-channel (0–255) difference at which a hex is treated as an exact
 * Tailwind color. Conservative: only snaps when the color is essentially the
 * palette value, so a custom color is preserved as an arbitrary hex.
 */
const SNAP_MAX_DELTA = 3;

export interface TailwindColorMatch {
  /** Palette token, e.g. "blue-500" or "white". */
  name: string;
  /** The largest per-channel difference between input and the matched color. */
  maxDelta: number;
}

/**
 * Custom palette loaded from the project's Tailwind config (v3 theme or v4
 * `@theme`). Brand colors here are matched BEFORE the built-in defaults so a
 * picked brand color resolves to its named class (e.g. `bg-brand-500`).
 */
let customPalette: Record<string, string> = {};

/** Replaces the project-specific color overrides (called by the theme loader). */
export function setCustomColors(palette: Record<string, string>): void {
  customPalette = palette;
}

/** Parses "#rrggbb" / "#rgb" → [r, g, b], or null if unparseable. */
...
/**
 * Finds the nearest Tailwind palette color to `hex`, returning its token and
 * the largest per-channel difference. Returns null only if `hex` is unparseable.
 *
 * Use `maxDelta <= SNAP_MAX_DELTA` (see {@link isExactTailwindColor}) to decide
 * whether to emit the named class or fall back to an arbitrary value.
 */
export function nearestTailwindColor(hex: string): TailwindColorMatch | null {
  ...
  // Custom (project) colors first so they win exact-distance ties over defaults.
  for (const [name, palHex] of [...Object.entries(customPalette), ...Object.entries(TAILWIND_PALETTE)]) {
    const rgb = parseHex(palHex);
    if (!rgb) continue;
...
/** True when `hex` is (essentially) a named Tailwind color. */
export function isExactTailwindColor(match: TailwindColorMatch | null): match is TailwindColorMatch {
  return match !== null && match.maxDelta <= SNAP_MAX_DELTA;
}
